...
We divided the training data of each player into 3 groups according to the distance range in **Table I**, and perform cross-validation for each group to find the appropriate features and the method to build the model
...
According to the CV results and the distance from the settlement center, we created 3 models for each player, and defined the input features and parameters (criterion, max depth=30) that the models should use.
...
- **F** means input features of tree
- **C** means criterion of tree
