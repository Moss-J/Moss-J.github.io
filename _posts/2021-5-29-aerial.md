...
We divided the training data of each player into 3 groups according to the distance range in **Table I**, and perform cross-validation for each group to find the appropriate Hyper-Parameters(inputing features, criterion, max depth of tree) to build the Decision Tree model.
...
According to the CV results, we created 3 models of different distance range for each player, and resulting Hyper-Parameters that the models should use.
